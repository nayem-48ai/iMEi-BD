...
export default function Sidebar({ isOpen, setIsOpen }) {
  ...
  const menuItems = [
    ...
    { name: 'Transfer IMEI', icon: 'bi-arrow-repeat', path: '/transfer' },
  ];
        ...
        <div className="p-4 mt-4">
            ...
            {menuItems.map((item) => (
                <Link key={item.path} href={item.path} onClick={() => setIsOpen(false)} 
                    className={`d-flex align-items-center p-3 mb-2 text-decoration-none rounded-3 ${pathname === item.path ? 'bg-white text-primary shadow' : 'text-white-50 hover-bg'}`}>
            ...
            {user ? (
                <Link href="/profile" onClick={() => setIsOpen(false)} className={`d-flex align-items-center p-3 mb-2 text-decoration-none rounded-3 ${pathname === '/profile' ? 'bg-white text-primary shadow' : 'text-white-50 hover-bg'}`}>
                    <i className="bi bi-person-circle fs-5 me-3"></i>
                    <span className="fw-semibold">My Profile</span>
                </Link>
            ) : (
                <Link href="/login" onClick={() => setIsOpen(false)} className={`d-flex align-items-center p-3 mb-2 text-decoration-none rounded-3 ${pathname === '/login' ? 'bg-white text-primary shadow' : 'text-white-50 hover-bg'}`}>
                    <i className="bi bi-box-arrow-in-right fs-5 me-3"></i>
